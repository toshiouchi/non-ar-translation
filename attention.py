...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, n_state: int, n_head: int, cross_attention: bool = False, kernel_size = [9,1], filter_size = 2048):
        ...
        self.cross_attn = MultiHeadAttention(n_state, n_head ).to(device) if cross_attention else None
        self.cross_attn_ln = LayerNorm(n_state).to(device) if cross_attention else None

        self.pos_ffn = PositionwiseFeedForward(n_state, d_hid = filter_size, kernel_size = kernel_size, dropout = 0.1  ).to(device)
    ...
    def forward(
        ...
        xx, attn1 = self.attn(self.attn_ln(x), mask=mask1)
        x = x + xx
        if self.cross_attn:
            xx, attn2 = self.cross_attn(self.cross_attn_ln(x).to(device), xa.to(device), mask = mask2.to(device))
            x = x + xx
        else:
            attn2 = []
        x = self.pos_ffn( x )
